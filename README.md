...
当前代码 schema 32 增加相似组低风险批量处理记录、可撤销快照与 5% 稳定抽检结论；这些状态只写入 SQLite，并随人工数据备份迁移。

数据库 schema 21 在 schema 20 的相似分组历史快照基础上增加 Capture 多维标签：题材、工作状态、人工问题和地点。常用标签随程序预置，自定义标签保存在 SQLite；人工、分析和导入来源彼此隔离，工作状态保持单选。schema 22 增加相册级拍摄附件记录，用于人工确认脚架、滤镜和闪光灯等 EXIF 无法可靠判断的器材。schema 23 将星级、选片入选/排除与后期工作状态明确拆分；旧的“精选/待淘汰”状态原样保留但不再允许新增。schema 24 为人工入选增加动作、表情、构图、关键瞬间和叙事补充等保留理由，用于建立多样性基准，不改变技术推荐。schema 25 增加只保存于 SQLite 的参数化修图方案历史。schema 26 增加选片会话活跃耗时和决策次数，只用于未来效率统计，不反推历史。schema 27 物化模型风险审计字段并保存完整性检查的可分页差异清单，使万张级图库默认按风险和抽样处理，而不是要求人工逐条查看。schema 28 为技术检测和模型风险增加轻量人工工作状态，使已核对、忽略、稍后或解决的结果退出默认队列，并在检测指纹变化后重新出现。schema 29 为完整性差异增加调查状态和跨检查重新出现判定；持续不变的已处理差异不会反复进入默认队列。schema 30 为后台任务失败增加按任务家族收敛的恢复状态；相同故障不会堆积，人工核对后再失败会重新进入队列，同类任务成功则自动解决。schema 31 增加按相册、题材、月份、置信度和问题类型均衡选取的固定模型审计基准，以及版本人工结论和扩大批次质量门禁。标签、附件、保留理由、修图方案、会话、待办、基准和调查状态都不写入照片。详情数据补全任务只读照片并写入 SQLite，支持暂停、继续和安全取消；不写 XMP、不修改原片，也不加载 GPU 模型。任何 schema 升级都会先创建并校验一致的 SQLite 备份，未来版本的数据库会在写入前被拒绝。迁移执行仍必须基于已审查的逐文件计划，并在网页中完整输入计划专属确认文字。任务会先检查全部源文件的大小和修改时间、目标冲突及当前可用空间，未通过时不会创建目标图库。
