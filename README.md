...
- 后台使用 SHA-256 做完整性核对；不作为日常图库入口，也不提供删除功能。
...
数据库 schema 14 在原有迁移恢复、活动图库状态、双重保护基线和模型审计能力上增加了可管理的相册类型。执行仍必须基于已审查的逐文件计划，并在网页中完整输入计划专属确认文字。任务会先检查全部源文件的大小和修改时间、目标冲突及当前可用空间，未通过时不会创建目标图库。
