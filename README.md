...
网页“应用设置”可随时修改照片、工作区、缓存、ExifTool、RAW 与本地模型配置。保存会先校验并在配置文件旁备份旧版本，然后原子替换配置；新配置在重启应用后生效，不会自动移动照片、数据库或缓存。
...
数据库 schema 21 在 schema 20 的相似分组历史快照基础上增加 Capture 多维标签：题材、工作状态、人工问题和地点。常用标签随程序预置，自定义标签保存在 SQLite；人工、分析和导入来源彼此隔离，工作状态保持单选。标签附着在 JPG+RAW 拍摄单元上，不写入照片。详情数据补全任务只读照片并写入 SQLite，支持暂停、继续和安全取消；不写 XMP、不修改原片，也不加载 GPU 模型。任何 schema 升级都会先创建并校验一致的 SQLite 备份，未来版本的数据库会在写入前被拒绝。迁移执行仍必须基于已审查的逐文件计划，并在网页中完整输入计划专属确认文字。任务会先检查全部源文件的大小和修改时间、目标冲突及当前可用空间，未通过时不会创建目标图库。
