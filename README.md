...
数据库 schema 19 在原有迁移恢复、活动图库状态、双重完整性基线和模型审计能力上增加了可管理相册类型、确认式相似组人工覆盖、查询索引、亮度直方图衍生数据及版本化扩展 EXIF。详情数据补全任务只读照片并写入 SQLite，支持暂停、继续和安全取消；不写 XMP、不修改原片，也不加载 GPU 模型。任何 schema 升级都会先创建并校验一致的 SQLite 备份，未来版本的数据库会在写入前被拒绝。迁移执行仍必须基于已审查的逐文件计划，并在网页中完整输入计划专属确认文字。任务会先检查全部源文件的大小和修改时间、目标冲突及当前可用空间，未通过时不会创建目标图库。
