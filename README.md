...
7. 不删除：低质量与近似照片只标记为排除，当前版本不提供删除能力。
...
- “质量与复盘”页面中的人工星级、精选和排除标记；这些可选的长期标记只写数据库，不要求逐张处理全库。
- 可单独启动 10 张快速验证，或按 25/50/100/200/500 张运行推荐批次；接口批次限制为 1–5,000 张，并显示逐张平均耗时、吞吐量、成功率和预计剩余时间。
- 相册默认把视觉相似连拍折叠为组卡片，也可展开全部；组卡片优先显示人工精选照片，并显示精选数量和总张数。
- Mac 隔离演示库预置连拍精选、多张精选、非连拍精选/排除和人工拆分样例；“全部照片”保持展开，进入相册详情后默认折叠。
- 相似组支持“从这里拆分”“移出本组”和“恢复自动分组”，人工覆盖绑定稳定照片 ID，视觉重建后仍然保留。
...
- 单张照片可按需标记精选或排除；连拍在组内对比后选择。批量勾选是退出即清空的临时操作，与持久的整理标记明确区分。
...
数据库 schema 15 在原有迁移恢复、活动图库状态、双重完整性基线和模型审计能力上增加了可管理相册类型及持久化相似组人工覆盖。执行仍必须基于已审查的逐文件计划，并在网页中完整输入计划专属确认文字。任务会先检查全部源文件的大小和修改时间、目标冲突及当前可用空间，未通过时不会创建目标图库。
