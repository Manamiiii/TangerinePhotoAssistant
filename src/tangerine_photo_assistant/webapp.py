...
from .album_routes import create_album_router
from .archive import (
...
class ScanTaskManager:
    ...
    def preview_album_archive(self, album_id: int) -> dict[str, Any]:
        with self._lock:
            if self._state.status in {'running', 'paused'} and self._state.stage != 'album-archive':
                raise ValueError('请先完成当前后台任务')
            with closing(connect_readonly(self.settings.database_path)) as connection:
                return album_archive.preview(connection, self.settings, album_id)
