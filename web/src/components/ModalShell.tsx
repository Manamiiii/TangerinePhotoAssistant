...
export function ModalShell({ title, close, children, wide = false, expanded = false }: {
  title: string;
  close: () => void;
  children: ReactNode;
  wide?: boolean;
  ...
  return <div className={`editor-backdrop ${expanded ? "is-expanded" : ""}`} onClick={close}>
    <section ref={modalRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={title} className={`editor-modal ${wide ? "wide" : ""} ${expanded ? "expanded" : ""}`} onClick={(event) => event.stopPropagation()}>
