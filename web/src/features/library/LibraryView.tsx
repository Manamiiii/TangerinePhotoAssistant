...
function PhotoLibraryView({ library, filters, query, updateQuery, openCapture, openGroup, editGrouping, exportPhotos, assignToAlbum, batchTag, changePage, changePageSize, albumContext = false, refreshLibrary }: {
  ...
  const activeFilters: Array<{ key: keyof LibraryQuery; label: string }> = [
    ...
    ...(query.selection ? [{ key: "selection" as const, label: query.selection === "picked" ? "已入选" : query.selection === "rejected" ? "已排除" : "未评价" }] : []),
  ...
  return <>
    <section className="library-filter-shell">
      <div className="library-filter-toolbar">
        ...
        <button className={`toolbar-button filter-toggle ${filtersOpen ? "active" : ""}`} onClick={() => setFiltersOpen((current) => !current)}>筛选{activeFilters.length ? <b>{activeFilters.length}</b> : null}</button><button className="toolbar-button tag-manager-entry" onClick={() => setTagManagerOpen(true)}>管理标签</button>
      ...
      {filtersOpen && <div className="library-filter-drawer">
        ...
        <fieldset><legend>评价</legend><label><span>人工星级</span><select value={query.rating} onChange={(event) => updateQuery({ rating: event.target.value })}><option value="">全部星级</option>{[5, 4, 3, 2, 1].map((rating) => <option key={rating} value={rating}>{rating} 星</option>)}</select></label><label><span>选片结论</span><select value={query.selection} onChange={(event) => updateQuery({ selection: event.target.value })}><option value="">全部结论</option><option value="picked">已入选</option><option value="rejected">已排除</option><option value="unreviewed">未评价</option></select></label><label><span>技术质量</span><select value={query.quality} onChange={(event) => updateQuery({ quality: event.target.value })}><option value="">全部质量</option><option value="problems">发现问题</option><option value="low">技术分低于 70</option><option value="high">技术分 85 以上</option><option value="unanalyzed">尚未分析</option></select></label><label><span>模型观察</span><select value={query.modelProblem} onChange={(event) => updateQuery({ modelProblem: event.target.value })}><option value="">全部观察</option>{(filters?.model_problems ?? []).map((problem) => <option key={problem.name} value={problem.name}>{problem.name}（{problem.capture_count}）</option>)}</select></label></fieldset>
        <fieldset><legend>标签</legend>{([['subject', 'tagSubject', '题材'], ['status', 'tagStatus', '工作状态'], ['problem', 'tagProblem', '人工问题'], ['location', 'tagLocation', '地点']] as Array<[CaptureTagDimension, keyof LibraryQuery, string]>).map(([dimension, key, label]) => <label key={dimension}><span>{label}</span><select value={String(query[key])} onChange={(event) => updateQuery({ [key]: event.target.value })}><option value="">全部</option>{(filters?.tags ?? []).filter((tag) => tag.dimension === dimension).map((tag) => <option key={tag.name} value={tag.name}>{tag.name}（{tag.capture_count}）</option>)}</select></label>)}<label><span>保留理由</span><select value={query.selectionReason} onChange={(event) => updateQuery({ selectionReason: event.target.value })}><option value="">全部</option>{["动作差异", "表情差异", "构图差异", "关键瞬间", "叙事补充"].map((reason) => <option key={reason}>{reason}</option>)}</select></label></fieldset>
