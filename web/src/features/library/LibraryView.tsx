  ...
  return <>
    <section className="library-filter-shell">
      ...
      {filtersOpen && <div className="library-filter-drawer"><div className="filter-drawer-heading"><span>筛选照片</span><button type="button" onClick={() => setTagManagerOpen(true)}>管理分类与状态</button></div>
        ...
        <fieldset><legend>评价</legend><label><span>人工星级</span><select value={query.rating} onChange={(event) => updateQuery({ rating: event.target.value })}><option value="">全部星级</option>{[5, 4, 3, 2, 1].map((rating) => <option key={rating} value={rating}>{rating} 星</option>)}</select></label><label><span>相似组选片</span><select value={query.selection} onChange={(event) => updateQuery({ selection: event.target.value })}><option value="">全部结论</option><option value="picked">已保留</option><option value="rejected">已排除</option><option value="unreviewed">未选择</option></select></label><label><span>技术质量</span><select value={query.quality} onChange={(event) => updateQuery({ quality: event.target.value })}><option value="">全部质量</option><option value="problems">发现问题</option><option value="low">技术分低于 70</option><option value="high">技术分 85 以上</option><option value="unanalyzed">尚未分析</option></select></label><label><span>模型观察</span><select value={query.modelProblem} onChange={(event) => updateQuery({ modelProblem: event.target.value })}><option value="">全部观察</option>{(filters?.model_problems ?? []).map((problem) => <option key={problem.name} value={problem.name}>{problem.name}（{problem.capture_count}）</option>)}</select></label></fieldset>
