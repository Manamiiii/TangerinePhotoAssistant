...
function workItemAgeLabel(days: number | null | undefined) {
  if (days == null) return "";
  if (days === 0) return " · 今天发现";
  return ` · 已积压 ${days} 天`;
}
    ...
    <div className="page-stack analysis-page">
      ...
      {analysisTab === "model" && <section className="panel ai-results-panel">
        ...
        {!!resultPage?.items.length && <div className="ai-result-grid">
          {resultPage.items.map((result) => <article key={result.id} className="ai-result-card">
            ...
            <div className="work-item-actions"><b>{({ new: "新发现", reappeared: "重新出现", pending: "待处理", confirmed: "已核对", ignored: "已忽略", snoozed: "稍后处理", resolved: "已解决" } as Record<string, string>)[result.workflow_status ?? "new"]}{workItemAgeLabel(result.workflow_age_days)}</b>{["new", "reappeared", "pending"].includes(result.workflow_status ?? "new") ? <><button onClick={() => void saveWorkItem("ai", result.id, "snoozed")}>7天后</button><button onClick={() => void saveWorkItem("ai", result.id, "ignored")}>忽略</button></> : <button onClick={() => void saveWorkItem("ai", result.id, "pending")}>重新打开</button>}</div>
          </article>)}
        </div>}
      ...
      {analysisTab === "quality" && (Boolean(qualityAlbumId) || qualityBrowseMode === "all") && <section className="panel quality-review-panel">
        ...
        <div className="quality-review-toolbar">
          ...
          <select value={qualityFilter} onChange={(event) => setQualityFilter(event.target.value as QualityReviewFilter)}><option value="problems">发现问题（建议优先）</option><option value="errors">读取失败</option><option value="low_score">技术健康度低于 70</option><option value="with_model">已有模型结果</option><option value="without_model">等待模型分析</option><option value="unrated">尚未评分</option><option value="all">全部已分析</option></select>
        ...
        <div className="quality-review-grid">
          {visibleQualityItems.map((item) => {
            ...
            const technicalSuggestion = item.has_error ? "请确认来源文件仍存在且可读取；处理来源问题后重新运行技术检测。" : item.issues[0] ? technicalAdvice(item.issues[0].code) : "当前技术指标正常，可结合构图和表达继续人工判断。";
            const workflowStatus = qualityStatusOverrides.get(item.capture_id) ?? item.workflow_status;
            const hasWorkItem = item.has_error || item.issues.length > 0;
            return (
            <article className="quality-review-card" key={item.capture_id}>
              {hasWorkItem && <label className="quality-work-select"><input type="checkbox" checked={qualitySelected.has(item.capture_id)} onChange={(event) => setQualitySelected((current) => { const next = new Set(current); event.target.checked ? next.add(item.capture_id) : next.delete(item.capture_id); return next; })} /><span>选择待办</span></label>}
              <button className="quality-review-photo" onClick={() => openCapture(item.capture_id, (quality?.items ?? []).map((entry) => entry.capture_id))}><img src={item.thumbnail_url} loading="lazy" alt={item.stem} /><span>{item.has_error ? "技术检测失败 · 等待处理" : `技术健康度 ${Math.round(item.technical_score)} · ${item.issues.length ? `${item.issues.length} 项需复核` : "未见明显故障"}`}</span></button>
