  ...
  return (
    <>
      <section className="structure-hero burst-hero">
        <div><span className="section-kicker">照片挑选</span><h2>相似照片分组</h2><button className="primary-action" onClick={startVisual} disabled={!task || task.status === "running" || task.status === "paused"}><span>{!task ? "正在读取任务状态" : task.status === "paused" ? "请先处理暂停任务" : task.status === "running" ? "任务进行中" : "更新相似分组"}</span><b aria-hidden="true">→</b></button></div>
      ...
      {selectedGroup ? (
        <section className="panel comparison-panel">
          ...
          <div className="comparison-note comparison-context"><span>共 {selectedGroup.capture_count} 张 · 点击图片查看完整参数</span><div className="comparison-context-actions"><GroupComparison key={selectedGroup.id} group={selectedGroup} /><div className="burst-view-toggle" role="group" aria-label="组内照片排序"><button className={comparisonOrder === "recommended" ? "active" : ""} onClick={() => setComparisonOrder("recommended")}>技术推荐</button><button className={comparisonOrder === "balanced" ? "active" : ""} onClick={() => setComparisonOrder("balanced")}>兼顾差异</button><button className={comparisonOrder === "capture" ? "active" : ""} onClick={() => setComparisonOrder("capture")}>拍摄顺序</button></div><button className="toolbar-button" onClick={() => setEditingGroupId(selectedGroup.id)}>调整这一组</button></div></div>
