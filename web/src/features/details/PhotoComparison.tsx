...
export function PhotoComparison({ photos, back, backLabel = "返回已选清单" }: { photos: [ComparePhoto, ComparePhoto]; back: () => void; backLabel?: string }) {
  ...
  return <div className={`photo-comparison layout-${layout}`}>
    ...
    <div className="comparison-layout-toolbar"><div className="comparison-layout-options" role="group" aria-label="对比布局">{([['side', '左右并排'], ['stack', '上下排列'], ['single', '单图切换']] as const).map(([value, label]) => <button key={value} className={layout === value ? "active" : ""} aria-pressed={layout === value} onClick={() => changeLayout(value)}>{label}</button>)}</div>{layout === "single" && <div className="comparison-photo-options" role="group" aria-label="当前对比照片">{photos.map((photo, index) => <button key={photo.id} className={active === index ? "active" : ""} aria-pressed={active === index} onClick={() => { release(); setActive(index); }}>{index === 0 ? "A" : "B"} · {photo.stem ?? photo.id}</button>)}</div>}</div>
