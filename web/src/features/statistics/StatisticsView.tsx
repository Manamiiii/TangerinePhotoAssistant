  ...
  return (
    <>
      ...
      <section className="metric-grid">
        ...
        <article><span>平均技术健康度</span><strong>{summary?.average_technical_score ?? "—"}</strong><small>只计算已有质量分析的照片</small></article>
      </section>
      <div className="statistics-view-toolbar"><div className="section-tabs" role="tablist" aria-label="统计视图">{([['overview', '概览'], ['parameters', '拍摄参数'], ['time', '成长趋势']] as const).map(([value, label]) => <button role="tab" aria-selected={statisticsView === value} key={value} className={statisticsView === value ? "active" : ""} onClick={() => setStatisticsView(value)}>{label}</button>)}</div>{statisticsView !== "time" && <div className="section-tabs" role="group" aria-label="统计值显示"><button aria-pressed={distributionMode === "count"} className={distributionMode === "count" ? "active" : ""} onClick={() => setDistributionMode("count")}>数量</button><button aria-pressed={distributionMode === "percent"} className={distributionMode === "percent" ? "active" : ""} onClick={() => setDistributionMode("percent")}>占比</button></div>}</div>
