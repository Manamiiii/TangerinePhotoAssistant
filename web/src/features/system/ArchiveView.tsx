...
type IntegrityPage = {
  ...
  limit: number;
  offset: number;
  items: Array<{ relative_path: string; status: string }>;
};
...
export function ArchiveView({ archive, activeLibrary, createBaseline, createActiveBaseline, checkIntegrity }: {
  ...
  const loadDifferences = async (scope: "archive" | "active", offset = 0) => {
    setDifferenceScope(scope);
    setDifferences(await getJson<IntegrityPage>(`/api/integrity/differences/${scope}?limit=50&offset=${offset}`));
  };
  ...
  const baselineCard = (title: string, status: ArchiveStatus | null, create: () => void, scope: "archive" | "active") => (
    <section className="panel archive-panel">
      ...
      {status?.baseline ? <div className="archive-status">
        ...
        {!!status.comparison?.samples.length && <><div className="integrity-samples">{status.comparison.samples.slice(0, 8).map((sample) => <div key={`${sample.status}-${sample.relative_path}`}><span>{sample.status}</span><strong>{sample.relative_path}</strong></div>)}</div><button className="toolbar-button" onClick={() => void loadDifferences(scope)}>查看全部差异</button></>}
  ...
  return <>
    ...
    {differenceScope && differences && <section className="panel archive-panel">
      <div className="panel-heading"><div><span className="section-kicker">完整差异清单</span><h3>{differenceScope === "archive" ? "历史存档" : "活动图库"} · {numberFormat.format(differences.count)} 项</h3></div><button className="toolbar-button" onClick={() => { setDifferenceScope(null); setDifferences(null); }}>关闭</button></div>
      <div className="integrity-samples">{differences.items.map((item) => <div key={`${item.status}-${item.relative_path}`}><span>{item.status}</span><strong>{item.relative_path}</strong></div>)}</div>
