...
2026-08-14 在正式 UI 验收完成后开始按上述顺序小步整理：质量照片/相册查询已移入
`queries/quality.py`，`webapp.py` 保留兼容包装和原 API 返回；前端只先抽离无状态的
`api.ts` 与 `formatters.ts`。页面组件、状态结构和接口契约均未改变，后续模块继续使用
同样的“小范围迁移 + 现有回归测试”方式推进。
