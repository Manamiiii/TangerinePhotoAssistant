...
以下保留初次静态审阅时的结论。后续使用隔离样例复现并修复；验证范围见文末。
...
- 导航在首次异步请求前取得代次，绑定来源上下文、取消旧请求并阻止重复翻页；页数据和详情
  就绪后一起更新，关闭后不接受迟到结果。图库分页与组内固定范围明确分离。
- 不再预加载或缓存可变详情 JSON；每次读取最新评价、标签和修图方案，并等待本窗口正在保存
  的星级请求。仅预热相邻两张图片，首次图片 URL 与预热一致，离开时释放预热引用。
- 图片加载、失败重试和拖动状态拆为独立查看器；缓存图片不被加载效果反向覆盖，换图清理
  重试定时器，回到 100% 归零平移，处理 pointer cancel/lost capture 和非被动滚轮监听。
- 全屏失败显示反馈；键盘聚焦控制栏可见。列表只保留行内相似组入口，去掉覆盖评价列的重复
  “展开组”；移除不适配列表行高的统一 content-visibility 占位。组内详情使用选片语义。
- Python 94 项、前端 25 项、Ruff、生产构建通过。新增 17 项前端测试覆盖固定范围、迟到请求、
  页与详情完整提交、每次读取新详情、相邻图片数量/清理、缓存图片判断、缩放归位和全屏失败。
  这些是逻辑单元测试，不等同于全套浏览器端到端测试。
- 经用户授权，使用已启动的正式服务做只读浏览：真实照片 DSCF3708/3707 缓存往返显示正常；
  拖动 `(200,70)` 后缩回 100% 为 `(0,0)`；DSCF3689 → DSCF3688 跨页并反向返回正常；
  关闭后焦点定位当前卡片，电脑全屏进入/退出正常，浏览期间控制台无 error/warn。
  1280px 列表中折叠组入口与评价列分离；代表图 DSCF3706 下一张为可见的 DSCF3705，
  不会隐式进入折叠成员。
- 正式健康接口为 `ok / local-only / schema 32`，任务空闲。未重启服务、修改人工评分/标签/
  分组/修图方案或启动分析。正式组均为待选，进入会记录选片会话，因此未进入其成员选片；
  组内边界和慢请求关闭使用逻辑测试验证。未声称完成 Mac Safari、故障图片浏览器注入、
  正式写后往返或本轮数据库备份完整性验收。
